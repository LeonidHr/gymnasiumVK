...
// Удаляем класс с массива
export function removeClassForArray(elSelector, activeClass) {
...
// Модуль "Сustom сursor" (в розробці) =======================================================================================================================================================================================================================
export function customCursor(double = false) {
	const wrapper = document.querySelector('[data-custom-cursor]');
	if (wrapper && !isMobile.any()) {
		wrapper.dataset.customCursor === 'double' ? double = true : false;
		let cursor1 = document.createElement('div'),
			cursor2,
			classes = {
				cursorOne: 'cursor-1',
				cursorSecond: 'cursor-2',
				mouseEnter: 'mouse-enter',
				mouseDown: 'mouse-down',
				noCursor: 'nocursor'
			};

		let toggleClass = (item, clazz, toggle) => toggle ? item.classList.add(clazz) : item.classList.remove(clazz);

		let getStyleSize = (item, width) => width ? Number.parseInt(window.getComputedStyle(item).width) : Number.parseInt(window.getComputedStyle(item).height);

		toggleClass(cursor1, classes.cursorOne, 'add');
		toggleClass(document.documentElement, classes.noCursor, 'add');
		toggleClass(document.body, classes.noCursor, 'add');

		wrapper.insertAdjacentElement('beforeend', cursor1);

		let cursorWidth1 = getStyleSize(cursor1, 'width'),
			cursorHeight1 = getStyleSize(cursor1),
			cursorWidth2,
			cursorHeight2;
		...
		if (double) {
			...
			toggleClass(cursor2, classes.cursorSecond, 'add');

			wrapper.insertAdjacentElement('beforeend', cursor2);

			cursorWidth2 = getStyleSize(cursor2, 'width');
			cursorHeight2 = getStyleSize(cursor2);
		}

		function mouseActions(e, posX, posY) {

			const { mouseEnter, mouseDown } = classes;

			if (e.type === 'mouseout') {
				cursor1.style.opacity += 0;
				double ? cursor2.style.opacity += 0 : false;
			} else if (e.type === 'mousemove') {
				cursor1.style.removeProperty('opacity');
				double ? cursor2.style.removeProperty('opacity') : false;
				posX = e.clientX;
				posY = e.clientY;
				if (e.target.closest('button') || e.target.closest('a') || e.target.closest('input') || (window.getComputedStyle(e.target).cursor !== 'none' && window.getComputedStyle(e.target).cursor !== 'default')) {
					toggleClass(cursor1, mouseEnter, 'add');
					double ? toggleClass(cursor2, mouseEnter, 'add') : false;
				} else {
					toggleClass(cursor1, mouseEnter);
					double ? toggleClass(cursor2, mouseEnter) : false;
				}
			} else if (e.type === 'mousedown') {
				toggleClass(cursor1, mouseDown, 'add')
				double ? toggleClass(cursor2, mouseDown, 'add') : false;
			} else if (e.type === 'mouseup') {
				toggleClass(cursor1, mouseDown)
				double ? toggleClass(cursor2, mouseDown) : false;
			}

			cursor1.style.transform = `translate3d(${posX - (cursorWidth1 / 2)}px, ${(posY) - (cursorHeight1 / 2)}px, 0)`;
			double ? cursor2.style.transform = `translate3d(${posX - (cursorWidth2 / 2)}px, ${(posY) - (cursorHeight2 / 2)}px, 0)` : false;
